...
public class StrategieBandit implements StrategieFighter{
    ...
    public void deplacement(ChampDeBataille champ, Fighter c) {
        ...
        for( int i = 0;i<pos.length; i++){
            if(champ.getGrid()[pos[i].getX()][pos[i].getY()].getContenue() instanceof Explosable){
                ...
                if( champ.getGrid()[pos[i].getX()][pos[i].getY()].getContenue()==null){


                 int k=  r.nextInt(4);

                  while  ( !(champ.getGrid()[pos[k].getX()][pos[k].getY()].getContenue()==null)){
                     k=  r.nextInt(4);
                  };
                  p = pos[k];
    ...
    public void combat(ChampDeBataille champ, Fighter c){
    ...
    public void action(ChampDeBataille champ, Fighter c) {
        Ia ia = new Ia(champ);
          decision = ia.applicationDecisionBandit(champ, c);
           System.out.println("Decision Bandit "+decision);
        switch(decision){
            case "deplacer":
                this.deplacement(champ, c);
                break;
